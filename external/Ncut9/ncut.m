...
try
    ...
    if strcmp(e.identifier, 'MATLAB:UndefinedFunction')
        compileDir_simple(fileparts(mfilename('fullpath')));
        W = sparsifyc(W,dataNcut.valeurMin);
    else
        rethrow(e);
    end
...
% [vbar,s,convergence] = eigs(@mex_w_times_x_symmetric,size(P,1),tril(P), nbEigenValues, 'largestreal',options); 
% [vbar,s] = eigs_new(@mex_w_times_x_symmetric,size(P,1),tril(P), nbEigenValues,'LA',options); % used by defaut
P=0.5*(P+P');
